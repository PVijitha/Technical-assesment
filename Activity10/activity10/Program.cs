...
int BordingPass3; Console.WriteLine("Enter 10 bording pass numbers");
for (int i = 0; i <= 2; i++)
{
...
Console.WriteLine(RandomNumber); if (RandomNumber.Equals(1))
{
    BordingPass.Dequeue();
    BordingPass.Enqueue(BordingPass3);
    Console.WriteLine($"Removed queue {RandomNumber}"); foreach (var item in BordingPass)
    {
...
else if (RandomNumber.Equals(2))
{
    ...
    Console.WriteLine($"Removed queue {RandomNumber}"); foreach (var item in BordingPass1)
    {
        Console.WriteLine($"items {item}");
    }
}
